...
public:
    ...
    void PreOrderPrint(RBTreeNode* root);
    RBTreeNode* getRoot() const{return root;}
private:
...
{
    RBTreeNode* insertNode = CreateNode(nData);
    RBTreeNode* insertNodeParent = nil;
    RBTreeNode* pTemp = root;
    //找到要插入的位置
    while( pTemp != nil)
    {
        insertNodeParent = pTemp;
    ...
    //这种情况即没有节点，插入节点为root节点
    if(insertNodeParent == nil)
        root = insertNode;
    //修改父节点的左右指针
    else if(nData < insertNodeParent->nData)
...
void RBTree::InsertFixup(RBTreeNode* pNode)
{
    while(pNode->parent->color == RED)
    {
        ...
        {
            RBTreeNode* pUNode = pNode->parent->parent->right;//pNode的叔叔节点
...
void RBTree::PreOrderPrint(RBTreeNode* tempRoot){
    if(NULL==tempRoot)
		return ;
	cout<<"("<<tempRoot->nData<<",";
	if(tempRoot->color)
	...
	PreOrderPrint(tempRoot->left);
	PreOrderPrint(tempRoot->right);
}
int main()
{
    try
